...
const export_validator_1 = require("../helpers/export-validator");
const ci_helper_1 = require("../helpers/ci-helper");
...
suite('Advanced Features E2E Tests', () => {
    ...
    setup(() => {
        ...
        exportValidator = new export_validator_1.ExportValidator();
    });
    teardown(async () => {
    ...
    suite('TC-E2E-012: Export Cancellation', () => {
        ...
        test('should cleanup temporary files after cancellation', async function () {
            ...
            const tempFiles = allFiles.filter(f => f.includes('.tmp') || f.includes('.temp') || f.includes('~'));
    ...
    suite('TC-E2E-013: Auto-Save Integration', () => {
        test('should auto-export when file is saved (if enabled)', async function () {
            ...
            const diagramPath = await fixtureManager.createMermaidFile(workspaceDir, 'auto-test.mmd', fixtureManager.createSimpleFlowchart());
            // Enable auto-export
        ...
        test('should not auto-export when disabled', async function () {
            ...
            const diagramPath = await fixtureManager.createMermaidFile(workspaceDir, 'no-auto.mmd', fixtureManager.createSimpleFlowchart());
    ...
    suite('TC-E2E-014: CodeLens Integration', () => {
        test('should provide CodeLens actions for mermaid files', async function () {
            ...
            const diagramPath = await fixtureManager.createMermaidFile(workspaceDir, 'codelens.mmd', fixtureManager.createSimpleFlowchart());
            // Open file
            const editor = await vscodeHelper.openFile(diagramPath);
            assert.ok(editor, 'File should be open');
            ...
            const codeLenses = await vscode.commands.executeCommand('vscode.executeCodeLensProvider', editor.document.uri);
        ...
        test('should provide CodeLens for markdown mermaid blocks', async function () {
            ...
            const mdPath = await fixtureManager.createMarkdownFile(workspaceDir, 'doc.md', [fixtureManager.createSimpleFlowchart()]);
    ...
    suite('TC-E2E-016: Complex Diagram Performance', () => {
        (0, ci_helper_1.skipInCI)('should export very complex diagram within timeout', async function () {
            this.timeout(40000);
            ...
            const diagramPath = await fixtureManager.createMermaidFile(workspaceDir, 'complex.mmd', complexDiagram);
        ...
        (0, ci_helper_1.skipInCI)('should handle extremely complex diagram gracefully', async function () {
            this.timeout(30000);
            ...
            const diagramPath = await fixtureManager.createMermaidFile(workspaceDir, 'extreme.mmd', extremeDiagram);
            await vscodeHelper.openFile(diagramPath);
            vscodeHelper.setupMockDialogs();
            vscodeHelper.setDefaultMockResponse('Yes');
            try {
    ...
    suite('TC-E2E-017: File Permission Errors', () => {
        (0, ci_helper_1.skipInCI)('should handle read-only output directory', async function () {
            this.timeout(20000);
            const workspaceDir = await fixtureManager.createTestWorkspace('readonly-test', []);
            const diagramPath = await fixtureManager.createMermaidFile(workspaceDir, 'test.mmd', fixtureManager.createSimpleFlowchart());
...
//# sourceMappingURL=advanced-features.test.js.map