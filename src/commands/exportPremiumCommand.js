...
exports.runPremiumExport = runPremiumExport;
const vscode = __importStar(require("vscode"));
const path = __importStar(require("path"));
const fs = __importStar(require("fs"));
const webExportStrategy_1 = require("../strategies/webExportStrategy");
const visualEnhancementManager_1 = require("../services/visualEnhancementManager");
const errorHandler_1 = require("../ui/errorHandler");
async function runPremiumExport(context) {
    const enhancementManager = new visualEnhancementManager_1.VisualEnhancementManager(context);
    const webExportStrategy = new webExportStrategy_1.WebExportStrategy(context);
    try {
        ...
        await vscode.window.withProgress({
            ...
            try {
                ...
                const result = await vscode.window.showInformationMessage(`✨ Premium export completed! Style: ${selectedStyle}`, 'Open File', 'Show in Explorer', 'Export Another Style');
                if (result === 'Open File') {
        ...
        errorHandler_1.ErrorHandler.logError(`Premium export failed: ${errorMsg}`);
        await vscode.window.showErrorMessage(`Premium export failed: ${errorMsg}`);
    }
}
async function showPremiumStylePicker(enhancementManager) {
...
async function showPremiumExportOptions(fileName) {
    // Format selection
    const formatItems = [
...
//# sourceMappingURL=exportPremiumCommand.js.map