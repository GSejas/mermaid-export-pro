...
exports.runBatchExport = runBatchExport;
...
async function runBatchExport(context, folderUri) {
    errorHandler_1.ErrorHandler.logInfo('Starting Mermaid Export Pro - Export Folder command...');
    try {
        ...
        const initialMaxDepth = config.get('batchExportDefaultDepth', 3);
        ...
        errorHandler_1.ErrorHandler.logInfo(logMessage);
        // Show initial discovery message when called from context menu
        if (folderUri && mermaidFiles.length > 0) {
            vscode.window.showInformationMessage(`Found ${mermaidFiles.length} mermaid diagram(s) in "${folderName}" ready for export folder`);
        }
        ...
        const formatText = exportOptions.allFormats ?
            `${exportOptions.allFormats.join(', ').toUpperCase()}` :
            exportOptions.format.toUpperCase();
        const proceed = await vscode.window.showWarningMessage(`Export ${finalMermaidFiles.length} mermaid files to ${formatText}?\n\n` +
            `📂 Source: ${path.basename(targetFolder)}\n` +
            `💾 Output: ${path.basename(outputDirectory)}\n` +
            `🎨 Theme: ${exportOptions.theme}`, { modal: true }, 'Export All', 'Cancel');
        if (proceed !== 'Export All') {
            return;
        }
        // Run Mermaid Export Pro - Export Folder
        const progressTitle = exportOptions.allFormats ?
            `Export Foldering ${finalMermaidFiles.length} files to ${exportOptions.allFormats.length} formats` :
        ...
        await vscode.window.withProgress({
            ...
            errorHandler_1.ErrorHandler.logInfo(`Using ${strategy.name} for Mermaid Export Pro - Export Folder`);
            let completed = 0;
            ...
            for (const file of finalMermaidFiles) {
                ...
                results.push(result);
                completed++;
                errorHandler_1.ErrorHandler.logInfo(`Exported ${fileName}: ${result.success ? 'SUCCESS' : 'FAILED'}`);
            }
            ...
            if (failed.length === 0) {
                vscode.window.showInformationMessage(`Mermaid Export Pro - Export Folder completed successfully! ${successful.length} files exported.`);
            }
            else {
                vscode.window.showWarningMessage(`Mermaid Export Pro - Export Folder completed with ${failed.length} failures. ${successful.length} files exported successfully.`);
            }
        });
    }
    catch (error) {
        errorHandler_1.ErrorHandler.logError(`Mermaid Export Pro - Export Folder failed: ${error}`);
 ...
 */
async function discoverMermaidFiles(directory, maxDepth = 5) {
    const mermaidFiles = [];
    ...
    errorHandler_1.ErrorHandler.logInfo(`Discovered ${mermaidFiles.length} mermaid files (max depth: ${maxDepth})`);
    return mermaidFiles;
}
/**
 ...
 */
async function getComprehensiveBatchExportOptions(sourceFolder, fileCount) {
    ...
    if (formatChoice.value === 'all') {
        ...
        if (!individualFormat)
            return null;
        selectedFormats = [individualFormat.value];
    }
    // === STEP 2: BACKGROUND COLOR SELECTION ===
    ...
    exportOptions.additionalFormats = selectedFormats.length > 1 ? selectedFormats.slice(1) : [];
    exportOptions.allFormats = selectedFormats;
    return {
        exportOptions,
        outputDirectory
    };
 ...
 */
async function getOutputDirectory(sourceFolder) {
 ...
 */
async function exportSingleFile(file, strategy, options, outputDirectory) {
 ...
 */
async function showBatchResults(results, outputDirectory, cancelled) {
    const successful = results.filter(r => r.success);
    const failed = results.filter(r => !r.success);
    // Generate report
    ...
    const action = await vscode.window.showInformationMessage(`Mermaid Export Pro - Export Folder ${cancelled ? 'cancelled' : 'completed'}. Report saved to ${path.basename(reportPath)}`, 'Open Report', 'Open Output Folder');
...
//# sourceMappingURL=batchExportCommand.js.map