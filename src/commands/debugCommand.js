...
exports.runDebugExport = runDebugExport;
...
let VisualEnhancementManager = null;
async function runDebugExport(context) {
    errorHandler_1.ErrorHandler.logInfo('Starting debug export command...');
    const testDiagrams = [
        {
            name: 'flowchart',
            simple: `flowchart TD
  ...
  root((Software Architecture))
    ...
    const testFormats = ['svg', 'png', 'jpg'];
    const complexityLevels = ['simple', 'complex'];
    const testOptions = {
    ...
    try {
        ...
        errorHandler_1.ErrorHandler.logInfo(`Created debug folder: ${debugRoot}`);
        const results = [];
                        ...
                        if (ENABLE_PREMIUM_TESTING && diagramPremiumFolder) {
                            if (format === 'svg') {
                                ...
                                for (const style of premiumStyles) {
                                    ...
                                    const premiumResult = await testPremiumStrategy(diagramContent, formatOptions, path.join(diagramPremiumFolder, style), context, style);
                                ...
                                const premiumResult = await testPremiumStrategy(diagramContent, formatOptions, path.join(diagramPremiumFolder, 'modern'), context, 'modern');
        ...
        const openFolder = await vscode.window.showInformationMessage(`Debug export completed! Results saved to: ${path.basename(debugRoot)}`, 'Open Debug Folder', 'View Report', 'Show in Explorer');
        ...
        errorHandler_1.ErrorHandler.logError(`Debug export failed: ${error}`);
        throw error;
    }
}
async function testCLIStrategy(diagram, options, outputFolder) {
    ...
    try {
        ...
        errorHandler_1.ErrorHandler.logInfo(`CLI export successful: ${outputPath}`);
    }
    catch (error) {
        ...
        errorHandler_1.ErrorHandler.logError(`CLI export failed: ${result.error}`);
    }
    result.duration = Date.now() - startTime;
    return result;
}
async function testWebStrategy(diagram, options, outputFolder, context) {
    const startTime = Date.now();
    const result = {
        strategy: 'Web',
        success: false,
    ...
    try {
        ...
        result.success = true;
        result.outputFiles.push(outputPath);
        errorHandler_1.ErrorHandler.logInfo(`Web export successful: ${outputPath}`);
    }
    catch (error) {
        result.error = error instanceof Error ? error.message : String(error);
        errorHandler_1.ErrorHandler.logError(`Web export failed: ${result.error}`);
    }
    result.duration = Date.now() - startTime;
    return result;
}
async function testPremiumStrategy(diagram, options, outputFolder, context, style) {
    const startTime = Date.now();
    const result = {
        strategy: 'Premium',
        success: false,
    ...
    try {
        ...
        errorHandler_1.ErrorHandler.logInfo(`Premium ${style} export successful: ${outputPath}`);
    }
    catch (error) {
        const errorMessage = error instanceof Error ? error.message : String(error);
        result.error = `Premium ${style} export failed: ${errorMessage}`;
        errorHandler_1.ErrorHandler.logError(`Premium ${style} export failed: ${result.error}`);
    }
    result.duration = Date.now() - startTime;
    return result;
}
async function generateDebugReport(debugRoot, diagrams, options, results) {
    const reportPath = path.join(debugRoot, 'debug.md');
...
`;
    ...
    errorHandler_1.ErrorHandler.logInfo(`Debug report generated: ${reportPath}`);
}
