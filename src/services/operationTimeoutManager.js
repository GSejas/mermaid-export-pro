...
exports.OperationTimeoutManager = void 0;
const vscode = __importStar(require("vscode"));
class OperationTimeoutManager {
    static instance;
    activeOperations = new Map();
    lastExportTime = 0;
    EXPORT_COOLDOWN_MS = 1000; // 1 second between exports
     ...
     */
    startOperation(id, name, type, progress, callbacks) {
     ...
     */
    async cancelOperation(id, reason = 'user') {
        const operation = this.activeOperations.get(id);
        if (!operation)
            return;
    ...
    async handleMediumTimeout(operation) {
        if (operation.isCompleted || operation.isCancelled)
            return;
        operation.progress.report({
        ...
        if (operation.callbacks.onMediumTimeout) {
            ...
            const choice = await vscode.window.showWarningMessage(`Export operation "${operation.config.name}" is taking unusually long (${Math.round((Date.now() - operation.startTime) / 1000)}s). This might indicate a problem.`, { modal: false }, 'Keep Waiting', 'Cancel Operation', 'Force Restart');
    ...
    async handleHardTimeout(operation) {
        if (operation.isCompleted || operation.isCancelled)
            return;
        operation.progress.report({
        ...
        // Force cancel
        await this.cancelOperation(operation.id, 'timeout');
        vscode.window.showErrorMessage(`Operation "${operation.config.name}" was forcefully cancelled due to timeout. This may indicate a system issue.`, 'Show Diagnostics').then(choice => {
            if (choice === 'Show Diagnostics') {
                this.showDiagnostics();
            }
        });
    }
    async handleNuclearTimeout(operation) {
        if (operation.isCompleted || operation.isCancelled)
            return;
        ...
        vscode.window.showErrorMessage(`Critical: Operation "${operation.config.name}" required emergency cleanup. All operations have been reset.`, 'Report Issue', 'Restart Extension').then(choice => {
    ...
    async restartOperation(operation) {
        // This would need to be implemented based on the specific operation type
        // For now, just cancel and let the user retry
        await this.cancelOperation(operation.id, 'timeout');
        vscode.window.showInformationMessage(`Operation "${operation.config.name}" was cancelled. Please try again.`, 'Retry Now').then(choice => {
            if (choice === 'Retry Now') {
...
exports.OperationTimeoutManager = OperationTimeoutManager;
//# sourceMappingURL=operationTimeoutManager.js.map