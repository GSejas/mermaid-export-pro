...
/**
 * Error pattern matchers for intelligent categorization
 */
...
interface ErrorStatistics {
  ...
  errorsByPhase: Map<BatchPhase, number>;
  retryableErrors: number;
  criticalErrors: number;
  lastError?: BatchExportError;
...
export class ErrorHandlingServiceImpl implements ErrorHandlingService {
  ...
  private recentErrors: BatchExportError[] = [];
  ...
  logError(error: BatchExportError): void {
    ...
    switch (error.severity) {
      case 'critical':
        ErrorHandler.logError(`CRITICAL: ${logMessage}`);
        break;
      case 'error':
        ErrorHandler.logError(logMessage);
        break;
      case 'warning':
        ErrorHandler.logWarning(logMessage);
        break;
    }
  }
  ...
  generateErrorReport(errors: BatchExportError[]): string {
    ...
    // Summary by category
    const categoryMap = new Map<string, BatchExportError[]>();
   ...
   */
  getRecentErrors(count: number = 10): BatchExportError[] {
  ...
  clearErrorHistory(): void {
    this.recentErrors = [];
    this.initializeStatistics();
    ErrorHandler.logInfo('Error history cleared');
  }
  ...
  private buildErrorContext(error: unknown, context: any, timestamp: Date): ErrorContext {
    return {
  ...
  private categorizeError(error: unknown, context: ErrorContext): BatchExportError {
    const message = this.extractErrorMessage(error);
    const category = this.classifyError(message);
    const pattern = this.errorPatterns.find(p => p.category === category);
    
    const batchError: BatchExportError = {
      code: category,
      message,
  ...
  private getErrorCategory(code: string): ErrorCategory {
    return (ErrorCategory as any)[code] || ErrorCategory.UNKNOWN;
  }

  private getContextualRecoveryActions(error: BatchExportError): string[] {
  ...
  private generateReproductionSteps(context: ErrorContext): string[] {
  ...
  private gatherSystemInfo(): SystemInfo {
    return {
  ...
  private updateStatistics(error: BatchExportError): void {
  ...
  private addToRecentErrors(error: BatchExportError): void {
  ...
  private logErrorByCategory(error: BatchExportError): void {
    ...
    if (error.severity !== 'warning' && error.recoveryActions.length > 0) {
      ErrorHandler.logInfo(`Recovery suggestions: ${error.recoveryActions.join('; ')}`);
    }
  }

  private formatErrorForLogging(error: BatchExportError): string {
    const parts = [];
 ...
 */
export const errorHandlingService = new ErrorHandlingServiceImpl();
